...
class CodeGenerator:
    ...
    for command in commands:
      if command[0] == "assign":
        ...
        if isinstance(condition, bool):
          if condition:
            self.generate(command[2])
          else:
            self.generate(command[3])
        else:
        ...
        self.prepare_const(bound_address, 'a')
        self.assembly.append("LOAD a")
        self.assembly.append("SUB f")  # iterator - lower_bound
  ...
  def calculate_expression(self, expression, target_reg='a', second_reg='b', third_reg='c', fourth_reg='d', fifth_reg='e', sixth_reg='f', seventh_reg='g', eighth_reg='h'):
    if expression[0] == "const":
      ...
      if expression[0] == "plus":
        ...
        self.assembly.append(f"SWAP {second_reg}")
        self.assembly.append(f"JZERO 32")
        self.assembly.append(f"SWAP {second_reg}")
        self.assembly.append(f"SWAP {third_reg}")
        self.assembly.append(f"JZERO 29")
        self.assembly.append(f"SWAP {third_reg}")
        self.assembly.append(f"RESET a")
        self.assembly.append(f"ADD {second_reg}")
    ...
    if type(index) == int:
      ...
      if type(index[1]) == tuple:
        if index[1][0] == "undeclared":
          ...
          # self.code.append(f"PUT")
          self.assembly.append(f"SWAP {second_reg}")
          var = self.memory.get_variable(array)
          self.prepare_const(var.memory_offset - var.first_index, target_reg)
          # print(var)
          self.assembly.append(f"SWAP {target_reg}")
          self.assembly.append(f"ADD {second_reg}")
          self.assembly.append(f"SWAP {target_reg}")
  ...
  def load_variable(self, name, target_reg, declared=True):
    ...
    if not declared and self.iterators and name == self.iterators[-1]:
      ...
      self.load_variable_address(name, target_reg, declared)
      self.assembly.append(f"LOAD {target_reg}")
      self.assembly.append(f"SWAP {target_reg}")

  def load_variable_address(self, name, target_reg, declared=True):
    if declared or name in self.iterators:
      address = self.memory.get_address(name)
      # print("address: ", address)
      self.prepare_const(address, target_reg)
